...
- The user selected poster option 2, `design/poster-concepts/orbital-workflow.png`, as the Hero source of truth. Preserve its empty left title zone, right-side crystalline core, silver orbital rings, finance-green/violet energy accents, and deep starfield. Hero motion should stay layered and restrained: slow shared starfield drift, a masked light sweep traveling around the real orbital artwork, and a subtle core-energy breath. This decision applies only to the Hero artwork and motion; do not use it as authorization to revise other chapters.
